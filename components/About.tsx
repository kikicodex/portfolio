...
function StatCard({ value, label, delay }: { value: number; label: string; delay: number }) {
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center gap-1 p-6 rounded-2xl border border-[#1e1e1e] bg-[#111111] hover:border-[#7c3aed]/40 transition-colors duration-300"
    >
      <span className="font-black text-6xl bg-gradient-to-br from-[#7c3aed] to-[#06b6d4] bg-clip-text text-transparent tabular-nums">
        {count}
      </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
          ...
          >
            ...
            <div className="flex flex-wrap gap-3 mt-2">
              {["AppSec Minded", "Solo Shipper", "Monorepo Architect", "AI Integrator", "Open to Remote"].map((t) => (
                <span
                  key={t}
